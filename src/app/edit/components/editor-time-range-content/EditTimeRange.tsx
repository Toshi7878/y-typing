...
const TimeRange = () => {
  ...
  const handleRangeChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    e.target.style.background = `linear-gradient(to right, ${theme.colors.primary.normal} ${progress}%, ${theme.colors.text.body}30 ${progress}%)`;
  ...
  useEffect(() => {
    const updateRangeValue = (currentTime: string) => {
      ...
      if (currentRangeRef) {
        ...
        rangeRef.current!.style.background = `linear-gradient(to right, ${theme.colors.primary.normal} ${progress}%, ${theme.colors.text.body}30 ${progress}%)`;
      }
    };
