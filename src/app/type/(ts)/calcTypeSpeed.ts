...
export class CalcTypeSpeed {
  ...
  constructor(status: Status, lineConstantTime: number, statusRef: React.RefObject<StatusRef>) {
    ...
    this.totalKpm = this.updateTotalKpm(status, lineConstantTime, statusRef);
  }

  updateTotalKpm(status: Status, lineConstantTime: number, statusRef: React.RefObject<StatusRef>) {
